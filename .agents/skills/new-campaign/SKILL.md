...
Create the following and **narrate it dramatically** to the player. Open with a **header banner** (see [`RULES.md` § "Display Formatting — Scene Banners"](RULES.md#display-formatting--scene-banners)) using the 🌟 MAJOR EVENT scene type for the world reveal. Then present the world using a **campaign intro card** (see [`RULES.md` § "Display Formatting — Campaign Intro Card"](RULES.md#display-formatting--campaign-intro-card)) showing the world name, conflict, magic level, starting locations, factions, and quest hook.
